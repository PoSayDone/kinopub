...
fun MainGraph() {
    ...
    var homeThemeSeedColor by remember { mutableStateOf<Color?>(null) }
    val currentEntry = topLevelBackStack.backStack.last()
    val destination = navBarScreenItems.any { it == currentEntry }
    val (drawer, firstItem) = remember { FocusRequester.createRefs() }

    HomeScreenDynamicTheme(
        seedColor = homeThemeSeedColor,
        enabled = currentEntry == NavBarGraphData.Home,
    ) {
        NavigationDrawer(
            modifier = Modifier
                ...
                entryProvider = entryProvider {
                    entry<NavBarGraphData.Home> {
                        HomeScreen(
                            ...
                            onThemeSeedColorChanged = { homeThemeSeedColor = it },
                        )
                    }
