...
internal fun AppUpdateDialog(
    ...
    val dialogTitle = when (state.stage) {
        AppUpdateStage.AVAILABLE,
    ...
    StandardDialog(
        ...
        dismissButton = if (primaryAction == null || primaryLabel == null) {
            null
        } else {
            {
        ...
        confirmButton = {
            Button(onClick = primaryAction ?: onDismiss) {
                Text(primaryLabel ?: stringResource(R.string.close))
            }
        },
    )
}
